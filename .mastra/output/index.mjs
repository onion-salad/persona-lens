...
import { weatherTool } from './tools/0c47f1b6-e097-4e93-8107-9f2612df2b0c.mjs';
...
async function savePersonaToSupabase(persona) {
...
const personaInputSchema = z.object({
  ...
  // agentによってはcontextを使わない場合もあるためoptionalにするか、呼び出し元で保証する
  // もし agent が直接 {personas: [...]} を渡す場合、これも許容するなら union type などを使う
  // 今回は orchestratorAgent が context 経由で渡すことを前提とする
  personas: z.array(z.object({
    // contextがない場合の直接的なpersonasも許容する場合（今回はorchestratorがcontext経由前提なのでコメントアウト）
    // industry: z.string(),
    // role: z.string(),
    // company_size: z.string(),
    // region: z.string(),
